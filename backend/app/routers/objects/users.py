...
@router.post('', response_model=UsersResponse, dependencies=[Depends(require_permission("users:create"))])
def create_users(
...
@router.put('/{object_id}', response_model=UsersResponse, dependencies=[Depends(require_permission("users:update"))])
def update_users(
    object_id: str,
    request: UsersUpsertRequest,
    controller: UsersController = Depends(get_controller),
...
@router.get('/{object_id}', response_model=UsersResponse, dependencies=[Depends(require_permission("users:read"))])
def get_users(
    object_id: str,
    tenant_id: str,
    controller: UsersController = Depends(get_controller),
...
@router.get('', response_model=list[UsersResponse], dependencies=[Depends(require_permission("users:read"))])
def list_users(
    tenant_id: str,
    controller: UsersController = Depends(get_controller),
) -> list[UsersResponse]:
